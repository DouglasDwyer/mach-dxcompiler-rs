use super::{fetch_release_json, json_field, release_json_is_immutable};
use super::{RELEASE_REPO_NAME, RELEASE_REPO_OWNER};
...
/// GitHub-computed `digest` field, and rewrites `targets.rs` to match.
pub fn run() -> ! {
    ...
    panic!(
        "Updated targets.rs for release `{tag}` with {} target(s). This always \"fails\" the \
         build; re-run `cargo build` without `--features internal_update_targets` to build \
         normally.",
        targets.len()
    );
}
...
fn render_targets_file(tag: &str, targets: &[(&str, bool, &str)]) -> String {
    ...
    let mut out = String::from(
        "// This file is automatically generated for the latest release using the \
         `internal_update_targets` feature\n\n\
         use crate::Target;\n\n\
